...
import { search } from "../scraping/spotify";
import { firecrawl } from "../firecrawl/spotify";
import { filterJobs } from "../../lib/openai/gpt";

export class SpotifyAudit implements Audit {
    ...
    async helper(browser: Browser, searchConfig: CompanySearchConfig): Promise<RawData> {
        ...
        if (source === "scraping") {
            ...
            const data = await search(browser, roleType);
